  ...
  entry: fs
    ...
    .filter((d) =>
      fs
        ...
        .find((d) => d === "visualization.config.json")
    )
...
function getPlugins() {
  class VisualizationDirectory {
    ...
    configFiles() {
      return ["visualization.config.json", "visualization.datashape.gql"];
    }
  }
  ...
  //
  VisualizationDirectories.forEach((d) => {
    if (d.containsEJS()) {
      d.configFiles().forEach((f) =>
    ...
    .map(
      (d) =>
        new TsconfigPathsPlugin({
          configFile: path.join(__dirname, "src", d, "tsconfig.json"),
        })
    );
}
